...
export const RS_CalculateTruthScore = StoreAccessor((claimID: string, calculationPath = [] as string[]): number => {
	return SubWatch('RS_CalculateTruthScore', [claimID, calculationPath], () => {
...
export const RS_CalculateTruthScoreComposite = StoreAccessor((argumentID: string, calculationPath = [] as string[]) => {
	return SubWatch('RS_CalculateTruthScoreComposite', [argumentID, calculationPath], () => {
...
export const RS_CalculateBaseWeight = StoreAccessor((claimID: string, calculationPath = [] as string[]) => {
...
export const RS_CalculateWeightMultiplier = StoreAccessor((nodeID: string, calculationPath = [] as string[]) => {
	return SubWatch('RS_CalculateWeightMultiplier', [nodeID, calculationPath], () => {
...
export const RS_CalculateWeight = StoreAccessor((argumentID: string, premiseIDs: string[], calculationPath = [] as string[]) => {
	return SubWatch('RS_CalculateWeight', [argumentID, premiseIDs, calculationPath], () => {
		const argument = GetNodeL2(argumentID);
...
export const RS_GetAllValues = StoreAccessor((nodeID: string, path: string, useRSPrefix = false, calculationPath = [] as string[]) => {
	return SubWatch('RS_GetAllValues', [nodeID, path, useRSPrefix, calculationPath], () => {
		const node = GetNodeL2(nodeID);
...
function GetChildArguments(node: MapNodeL2): MapNodeL3[] {
